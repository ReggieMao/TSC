...
public class SwipeLayoutTopAndDel extends LinearLayout {

    private static final String TAG = SwipeLayoutTopAndDel.class.getSimpleName();
    ...
    public SwipeLayoutTopAndDel(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    static List<SwipeLayoutTopAndDel> swipelayouts = new ArrayList<>();
        public  static void addSwipeView(SwipeLayoutTopAndDel v){
            if(null==v){
                return;
            }
            swipelayouts.add(v);
        }
        public static void removeSwipeView(SwipeLayoutTopAndDel v){
            if(null==v){
                return;
            }
            v.SimulateScroll(SwipeLayoutTopAndDel.SHRINK);
        }
        private void shrinkAllView(){
            for(SwipeLayoutTopAndDel s :swipelayouts){
                if(null==s){
                    swipelayouts.remove(s);
                    continue;
                }else {
                    s.SimulateScroll(SwipeLayoutTopAndDel.SHRINK);
                }
